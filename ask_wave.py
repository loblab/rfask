...
class BitWave:
    ...
    def show(self):
        ...
        print('[WAV] %.1fms/%d/%d:' % (dur * 1000, self.startbit, size), end=' ')
        for i in range(0, size):
          print('%.2f' % ((self.timestamp[i + 1] - self.timestamp[i]) * 1000), end=' ')
        print

    def dither(self, r=5e-5):
        self.timestamp = list(map(lambda x:x+random.random()*r, self.timestamp))
